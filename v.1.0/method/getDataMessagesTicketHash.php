<?php
/*
Метод для получения сообщений по тикету поддержки
*/
...
if (isset($hash_tiket_support) && isset($id_user_tboil)) {
      ...
      $check_valid_token = $settings->validate_token($token,$resource);
                           $settings->recording_history($resource,'getDataMessagesTicketHash',$check_valid_token);

      if (json_decode($check_valid_token)->response) {
            $check_id_referer = $settings->get_data_referer($resource);
            if (json_decode($check_id_referer)->response) {
                           $response = $settings->get_support_messages_tiket($hash_tiket_support,$id_user_tboil);
                           $settings->recording_history($resource,'getDataMessagesTicketHash',$response);
                           echo $response;
            } else {
