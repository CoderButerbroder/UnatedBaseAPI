<?php
/*
Метод для получения данных по юр.лицу резиденту ЛПМ из единой базы данных по ИНН компании в Единой базе данных
$id_entity = id юр лица в единой базе данных
*/
...
if ($inn) {
      ...
      $check_valid_token = $settings->validate_token($token,$resource);
                           $settings->recording_history($resource,'getLPMEntity',$check_valid_token);

      if (json_decode($check_valid_token)->response) {
              $response = $settings->search_resident_lpm($inn);
                          $settings->recording_history($resource,'getLPMEntity',$response);
              echo $response;
      } else {
