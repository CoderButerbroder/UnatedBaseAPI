...
if ($hash_tiket_support && $id_user_tboil && $message && $type_user) {
      ...
      if (json_decode($check_valid_token)->response) {
          ...
          if (json_decode($check_id_referer)->response) {
              $response = $settings->add_new_support_messages($hash_tiket_support, $id_user_tboil, $message, json_decode($check_id_referer)->data->id, $type_user);
      ...
      echo json_encode(array('response' => false, 'description' => 'Не все обязательные поля были заполнены для добавления сообщения в единую базу данных', 'data_referer' => $_POST),JSON_UNESCAPED_UNICODE);
      exit;
}
