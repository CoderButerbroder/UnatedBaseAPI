...
if($order_num_request < 0 || $order_num_request > 4) $flag_error = true;
...
$get_data = $settings->get_event_datatable($order_request, $type_order_request, $limit_start, $limit_count, $searh_value);
...
foreach ($get_data->data as $key => $value) {
  ...
  $temp_obj_data->Number = $value->id;
  $temp_obj_data->Name = $value->name;
  $temp_obj_data->Place = $value->place;
  $temp_obj_data->Data = date('H:i d.m.Y', strtotime( $value->start_datetime_event ));
