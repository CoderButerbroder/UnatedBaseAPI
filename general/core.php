...
class Settings {
  ...
  public function get_log_api_response($increment=true,$period='data', $start=NULL, $end=NULL) {
      global $database;
  ...
  public function get_log_api_response_group_by($increment=true,$period='data', $start=NULL, $end=NULL) {
      ...
      if ($period == 'year') {
          $strokaSQL .= " GROUP BY $this->history.`method`, YEAR($this->history.`date_request`)
                          HAVING SUM($this->history.`id`) > 0
                          ORDER BY YEAR($this->history.`date_request`) ASC";
      }
      if ($period == 'month') {
          $strokaSQL .= " GROUP BY $this->history.`method`, MONTH($this->history.`date_request`), YEAR($this->history.`date_request`)
                          HAVING SUM($this->history.`id`) > 0
                          ORDER BY YEAR($this->history.`date_request`), MONTH($this->history.`date_request`) ASC";
      }
      if ($period == 'week') {
          $strokaSQL .= " GROUP BY $this->history.`method`, WEEK($this->history.`date_request`), YEAR($this->history.`date_request`)
                          HAVING SUM($this->history.`id`) > 0
                          ORDER BY YEAR($this->history.`date_request`), WEEK($this->history.`date_request`) ASC";
      }
      if ($period == 'day') {
          $strokaSQL .= " GROUP BY $this->history.`method`, DAY($this->history.`date_request`),MONTH($this->history.`date_request`), YEAR($this->history.`date_request`)
                          HAVING SUM($this->history.`id`) > 0
                          ORDER BY YEAR($this->history.`date_request`), MONTH($this->history.`date_request`),DAY($this->history.`date_request`) ASC";
      }
      if ($period == 'data') {
  ...
  public function get_count_main_entity_fci_groupby_time_reg($increment=true,$period='data',$start=NULL,$end=NULL) {
        ...
        if ($period == 'year') {
              $strokaSQL .= " count(DISTINCT($this->MAIN_entity.`inn`)) as sum,
                              ...
                              count(DISTINCT($this->MAIN_entity.`inn`)) as fci_groupby";
        }
        if ($period == 'month') {
              $strokaSQL .= " count(DISTINCT($this->MAIN_entity.`inn`)) as sum,
        ...
        if ($period == 'year') {
            $strokaSQL .= " GROUP BY YEAR(date_register)
                            ...
                            ORDER BY YEAR(date_register) ASC";
        }
        if ($period == 'month') {
            $strokaSQL .= " GROUP BY MONTH(date_register), YEAR(date_register)
                            HAVING SUM($this->MAIN_entity.`inn`) > 0
                            ORDER BY YEAR(date_register), MONTH(date_register) ASC";
        }
        if ($period == 'week') {
            $strokaSQL .= " GROUP BY WEEK(date_register), YEAR(date_register)
                            HAVING SUM($this->MAIN_entity.`inn`) > 0
                            ORDER BY YEAR(date_register), WEEK(date_register) ASC";
        }
        if ($period == 'day') {
            $strokaSQL .= " GROUP BY DAY(date_register),MONTH(date_register), YEAR(date_register)
                            HAVING SUM($this->MAIN_entity.`inn`) > 0
                            ORDER BY YEAR(date_register), MONTH(date_register),DAY(date_register) ASC";
        }
        if ($period == 'data') {
            $strokaSQL .= " ";
        }
  ...
  // количеcтво физ. лиц участников программ ФСИ на платформе (нараст.итог)
  public function get_count_main_entity_fci_program_groupby_time_reg($increment=true,$prefix_program='all',$period='data',$start=NULL,$end=NULL) {
        global $database;
  ...
  // Количество юр. лиц - участников Сколково на платформе (нараст.итог) + прирост к предыдщему месяцу в процентах
  public function get_count_main_entity_skolkovo_groupby_time_reg($increment=true,$period='data',$start=NULL,$end=NULL) {
        global $database;
        ...
        if ($period == 'year') {
              $strokaSQL .= " count(DISTINCT($this->MAIN_entity.`inn`)) as sum,
                              YEAR($this->MAIN_entity.`date_register`) as yeard,
                              count(DISTINCT($this->MAIN_entity.`inn`)) as skolkovo_groupby";
        }
        if ($period == 'month') {
              $strokaSQL .= " count(DISTINCT($this->MAIN_entity.`inn`)) as sum,
                              MONTH($this->MAIN_entity.`date_register`) as monthd,
        ...
        $strokaSQL .= " FROM $this->IPCHAIN_StateSupport
                        INNER JOIN $this->IPCHAIN_entity ON $this->IPCHAIN_StateSupport.`ipchain_id_entity` = $this->IPCHAIN_entity.`id`
                        INNER JOIN $this->MAIN_entity ON $this->MAIN_entity.`inn` = $this->IPCHAIN_entity.`inn`
                        WHERE  $this->IPCHAIN_StateSupport.`typeId` NOT LIKE ' ' AND $this->MAIN_entity.`date_register` BETWEEN :starting AND :ending ";
  ...
  // Функция получения количества заявок по сервисам поданых в ЛК технопарка
  public function get_count_main_support_ticket_groupby_time_add($increment=true,$type_support='all',$period='data',$start=NULL,$end=NULL) {
        global $database;
        ...
        if ($period == 'year') {
              $strokaSQL .= " count(type_support) as sum,
                              YEAR(date_added) as yeard,
                              count(type_support) as support_ticket_groupby";
        }
        if ($period == 'month') {
              $strokaSQL .= " count(type_support) as sum,
                              MONTH(date_added) as monthd,
                              YEAR(date_added) as yeard,
                              count(type_support) as support_ticket_groupby";
        }
        if ($period == 'week') {
              $strokaSQL .= " count(type_support) as sum,
                              WEEK(date_added) as weekd,
                              YEAR(date_added) as yeard,
                              count(type_support) as support_ticket_groupby";
        }
        if ($period == 'day') {
              $strokaSQL .= " count(type_support) as sum,
                              DAY(date_added) as dayd,
                              MONTH(date_added) as monthd,
                              YEAR(date_added) as yeard,
                              count(type_support) as support_ticket_groupby";
        }
        if ($period == 'data') {
          ...
          if ($type_support != 'all') {
            $strokaSQL .= " FROM $this->MAIN_support_ticket
                            WHERE `type_support` = :type_support AND `date_added` BETWEEN :starting AND :ending ";
          }
          else {
            $strokaSQL .= " FROM $this->MAIN_support_ticket
                            WHERE `date_added` BETWEEN :starting AND :ending ";
          }

        if ($period == 'year') {
            $strokaSQL .= " GROUP BY YEAR(date_added)
                            HAVING SUM(id_tboil) > 0
        ...
        $statement = $database->prepare($strokaSQL);
        if ($type_support != 'all') {
          $statement->bindParam(':type_support', $type_support, PDO::PARAM_STR);
        }
        $statement->bindParam(':starting', $start, PDO::PARAM_STR);
        $statement->bindParam(':ending', $end, PDO::PARAM_STR);
        $statement->execute();
  ...
  // Получение количества мероприятий по периодам год/месяц/неделя/день и промежутку времени, а так же общее суммирование мероприятий по периодам и промежутку времени
  public function get_count_main_events_groupby_time_add($increment=true,$period='data',$start=NULL,$end=NULL) {
        global $database;
        ...
        array_push($data_users, (object) array('summ_all' => $count_all_users));
  ...
  /* экстренные функции для исправления ошибок в базе данных */


  // УДАЛЕНИЕ дубликатов физических лиц
  public function get_dubble_users() {
      global $database;

      $response = $database->prepare("SELECT MIN(`id`) AS `id` FROM $this->main_users GROUP BY `email`");
      $response->execute();
      $data = $response->fetchAll(PDO::FETCH_OBJ);

      $response = $database->prepare("SELECT * FROM $this->main_users");
      $response->execute();
      $data2 = $response->fetchAll(PDO::FETCH_OBJ);

      $arrayone = array();
      $arraytwo = array();

      foreach ($data as $key => $value) {
            array_push($arrayone,$value->id);
      }

      foreach ($data2 as $key => $value) {
            array_push($arraytwo,$value->id);
      }

      $result = array_diff($arraytwo,$arrayone);

      if (count($result) > 0) {

            $masiv_for_delete = implode(',',$result);

            $strokaSQL = "DELETE FROM $this->main_users WHERE id IN ($masiv_for_delete)";

            $response = $database->prepare($strokaSQL);
            $response->execute();
            ...
            $response = $database->prepare("SELECT * FROM $this->main_users");
            $response->execute();
            $count_users_itog = $response->fetchAll(PDO::FETCH_OBJ);

            $data_user_count = count($data2) - count($count_users_itog);

            if (count($result) == $data_user_count) {
                  return json_encode(array('response' => true,  'description' => 'Все дублирующиеся пользователя были удалены, удалено '.$data_user_count),JSON_UNESCAPED_UNICODE);
                  exit;
            }
            else {
                  return json_encode(array('response' => false,  'description' => 'Не все дубликаты были удалены, из '.count($result).' удалено '.$data_user_count),JSON_UNESCAPED_UNICODE);
                  exit;
            }

      } else {
          return json_encode(array('response' => false,  'description' => 'Нет дублирующихся пользователей на удаление'),JSON_UNESCAPED_UNICODE);
          exit;
      }
