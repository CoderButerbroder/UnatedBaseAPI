  ...
  public function auth_user_social($data) {
      ...
      if ($user) {
            ...
            if (!$user_check_social) {
                ...
                $new_uruser = $database->prepare("INSERT INTO $this->main_users (email,password,phone,name,last_name,second_name,DOB,photo,adres,inn,passport_id,id_entity,position,hash) VALUES (:email,:password,:phone,:name,:last_name,:second_name,:DOB,:photo,:adres,:inn,:passport_id,:id_entity,:position,:hash)");
                ...
                $id_new_user = $new_uruser->lastInsertId();
                ...
                if ($count) {
                      ...
                      if ($count) {
                            return json_encode(array('response' => true, 'description' => 'Пользотватель успешно создан',  'data' => json_decode($this->get_cur_user($hash))->data),JSON_UNESCAPED_UNICODE);
                      } else {
